...
def save_chat_result(chat_result, filename):
    """Save ChatResult to a JSON file."""
    with open(filename, "w", encoding="utf-8") as f:
        json.dump(chat_result.model_dump(), f, ensure_ascii=False)

class LoggingChatOpenAI(ChatOpenAI):
    def _generate(self, messages, stop=None):
        # Call the parent class method to generate the model response
        response = super()._generate(messages, stop)

        # Log or print the response here
        print("Model Response:", response)
        save_chat_result(response, "chat_result.json")
        return response
